...
function sassy_starter_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'sassy-starter' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'sassy-starter' ),
...
function sassy_starter_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'sassy-starter' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
...
function sassy_starter_categorized_blog() {
	...
	if ( $all_the_cool_cats > 1 ) {
		...
		// This blog has only 1 category so sassy_starter_categorized_blog should return false.
		return false;
	}
}
